...
var endpointURL = "http://bolegweb.geof.unizg.hr/geoserver/proplant/ows";
var featureTYPE = "proplant:obs_all";
...
var default_plnt_name = 'Plant name not defined';
...
var observationsInitialOffset = 0;
...
var MainWindow = new Window("Proplant monitor", true);
MainWindow.open();
...
function drawObservationsToMainWindow() {
	...
	observationsTop = 240 + observationsInitialOffset;
	...
	// observacie z DB
	var db = Ti.Database.open(menoDatabazy);
	var select = db.execute('SELECT * from annotations WHERE gmlID=0 AND uid=\''+myNickname+'\' ORDER BY id DESC');
	...
	// observacie z DB
	var db = Ti.Database.open(menoDatabazy);
	var select = db.execute('SELECT * from annotations WHERE mark_for_push=1 AND uid=\''+myNickname+'\' ORDER BY id DESC');
	...
	// observacie z DB
	var db = Ti.Database.open(menoDatabazy);
	var select = db.execute('SELECT * from annotations WHERE mark_for_del=1 AND uid=\''+myNickname+'\' ORDER BY id DESC');
...
function redrawObservations() {
    ...
    var db = Ti.Database.open(menoDatabazy);
    var select = db.execute('SELECT * from annotations WHERE uid=\''+myNickname+'\' AND (gmlID=0 OR mark_for_push=1 OR mark_for_del=1) ');
    db.close();
...
function pushOneImage(){
    ...
    var db = Ti.Database.open(menoDatabazy);
    var s1='SELECT * from annotations WHERE uid=\''+myNickname+'\' AND imageDeviceId=\''+myDeviceId+'\' AND imagePathLocal!=\'\' AND imagePathLocal!=\'null\' AND (imagePathServer=\'\' OR imagePathServer=\'null\') LIMIT 1';
    var s2='SELECT * from annotations WHERE uid=\''+myNickname+'\' AND imageDeviceId=\''+myDeviceId+'\' AND imagePathLocal!=\'\' AND imagePathLocal!=\'null\' AND (imagePathServer=\'\' OR imagePathServer=\'null\')';
    ...
    if(select.rowCount)
    {
        ...
        var url = 'http://bolegweb.geof.unizg.hr/proplant/api/push_photo.php';
        ...
        xhr.onreadystatechange = function (e) {
            ...
            if (xhr.readyState == 4) {
                ...
                if (xhr.status == 200) {
                    ...
                    var db = Ti.Database.open(menoDatabazy);
                    var myNicknamePart = myNickname.split("#");
                    myNicknamePart = myNicknamePart[0];
	...
	if (isOnline) {
		...
		try {
			...
			xhr.onload = function () {
				...
				//$.buttonAddToMap.hide();
				featureTYPE = "proplant:obs_all"; // this is default value for selection
				//label.show();
			};
				...
				if (xhr.readyState == 4) {
					if (xhr.status == 200) {
						...
						if (pocet > 500) {
							...
							alert("WARNING: too many results, only first 500 will be added to map");
						}
						...
						for (var i = 0; i < pocet; i++) {
							...
							if (select.rowCount) {
								...
								db.execute("UPDATE annotations SET lat=" + 
                                    lat + 
                                    ", lon=" + 
                                    lon + 
                                    ", featureType='" + 
                                    ...
                                    "', site_descr='" + 
                                    site_descr + 
                                    "', mark_for_del=0, mark_for_push=0 WHERE gmlID='" + 
                                    gmlID + 
                                    "' ");
							} else {
								// insert do db
								db.execute("INSERT INTO annotations (id, lat, lon, featureType, endpointURL, gmlID, plnt_name, image, imageDeviceId, imagePathLocal, imagePathServer, obs_date, uid, crtn, site_conf, site_descr) VALUES (" + 
                                    featIdIn + 
                                    "," + 
                                    lat + 
                                    "," + 
...
function eventListenerMainWindow() {
	...
	MainWindow.activity.onCreateOptionsMenu = function (e) {
		...
		readdEventListener(overflowItem, 'expand', function () {
			...
			var dataSet = ['Local Data', 'Server Data', 'Open Map', 'Support', 'About'];
...
function eventListenerFromWebView2(e) {
	...
	var gmlID = 0;
	var plnt_name = default_plnt_name;
	var photo = '';
	...
	// INSERT INTO DB
	var db = Ti.Database.open(menoDatabazy);
	db.execute("INSERT INTO annotations (id, lat, lon, featureType, endpointURL, gmlID, plnt_name, image, obs_date, uid, crtn, site_conf, site_descr) VALUES (" + featIdIn + "," + lat + "," + lon + ", '" + featureTYPE + "', '" + endpointURL + "', '" + 0 + "', '" + plnt_name + "', '" + photo + "', '" + obs_date + "','" + uid + "', '" + crtn + "', '" + site_conf + "', '" + site_descr + "' ) ");
	db.close();
	featIdIn = featIdIn + 1;
	buttonLocalDataFunc();
...
function eventListenerFromWebView(e) {
	 ...
	 */
		...
		var WinEdit = new Window(title, false, true, function (evt) {
				...
				if (evt.source.title == 'Save') {
                    ...
                    var sqlString = "UPDATE annotations SET plnt_name='%1', obs_date='%3', crtn='%4', site_conf='%5', site_descr='%6', mark_for_del='0' WHERE id='" + currentFeatIdIn + "'";
                        sqlString = formatString(sqlString, [textFieldPlntName.html, cameraImageName, textFieldObsDate.value, textFieldCrtn.value, textFieldSiteConf.value, textFieldSiteDescr.value, imagePathLocal, imagePathServer]);
	...
	top = 60;
	textFieldPlntName = new Label(select.fieldByName('plnt_name'));
	...
	if (myNickname == select.fieldByName('uid')) {
		readdEventListener(textFieldPlntName, "click", function () {
			...
			var searchView = Ti.UI.Android.createSearchView({
                hintText: "Plant search",
                backgroundColor: '#000000'
            });

			var plnt_name_window = new Window("Latin plant names");
			plnt_name_window.open();
			
			plnt_name_window.addEventListener('open', function(){
			    
			    plnt_name_window.activity.actionBar.hide();
			    
			});
			
			var plnt_name_list = new List("", dataListPlantLatinName, function (evt) {
                plnt_name_window.close();
                textFieldPlntName.html = dataListPlantLatinName[evt.itemIndex];
            ...
            plnt_name_window.add(plnt_name_list);
	...
	if (imagePathServer !== null && imagePathServer!="") {
		if (imagePathLocal !== null && imagePathLocal!="") {
	...
	top += posun;
	textFieldSiteDescr = new TextField(select.fieldByName('site_descr'));
	...
	if (Ti.Geolocation.locationServicesEnabled) {
		...
		Titanium.Geolocation.getCurrentPosition(function (e) {
			if (e.error) {
				...
				var site_descr = '';
				var db = Ti.Database.open(menoDatabazy);
				db.execute("INSERT INTO annotations (id, lat, lon, acc, alt, ala, featureType, endpointURL, gmlID, plnt_name, image, obs_date, uid, crtn, site_conf, site_descr) VALUES (" + featIdIn + "," + lat + "," + lon + "," + acc + "," + alt + "," + ala + ", '" + featureTYPE + "', '" + endpointURL + "', '" + 0 + "', '" + plnt_name + "', '" + photo + "', '" + obs_date + "','" + uid + "', '" + crtn + "', '" + site_conf + "', '" + site_descr + "' ) ");
				db.close();
				featIdIn = featIdIn + 1;
				buttonLocalDataFunc();
				var centerMap = 'zoomOnGPS(' + lon + ',' + lat + ');';
...
function selectedObservationsToBoxes(select, noDataText, kind) {
	...
	while (select.isValidRow()) {
		...
		var observation1plantName = new Label(select.fieldByName('plnt_name'), null, 10);
...
function drawPushWindow(){
    ...
    var db = Ti.Database.open(menoDatabazy);
    var select = db.execute('SELECT * from annotations WHERE uid=\''+myNickname+'\' AND gmlID=0 ');
    db.close();
    ...
    var db = Ti.Database.open(menoDatabazy);
    var select = db.execute('SELECT * from annotations WHERE uid=\''+myNickname+'\' AND mark_for_push=1 ');
    db.close();
    ...
    var db = Ti.Database.open(menoDatabazy);
    var select = db.execute('SELECT * from annotations WHERE uid=\''+myNickname+'\' AND mark_for_del=1 ');
    db.close();
    ...
    var db = Ti.Database.open(menoDatabazy);
    var select = db.execute('SELECT * from annotations WHERE uid=\''+myNickname+'\' AND imageDeviceId=\''+myDeviceId+'\' AND imagePathLocal!=\'\' AND imagePathLocal!=\'null\' AND (imagePathServer=\'\' OR imagePathServer=\'null\') ');
    db.close();
    ...
    var pushViewButtonPush = new Button("Push all", function(){
        ...
        dialog.addEventListener('click', function (e) {
            ...
            if (e.index === e.source.cancel) {
                ...
                var tbdi = 0;
                var db = Ti.Database.open(menoDatabazy);
                var select = db.execute('SELECT * FROM annotations WHERE mark_for_del=1 AND uid=\''+myNickname+'\'');
                db.close();
                while (select.isValidRow()) {
                            ...
                            if (xhr.readyState == 4) {
                                if (xhr.status == 200) {
                                    ...
                                    if (obj != 0) {
                                        ...
                                        if (obj == "deletedOK") {
                                            var db = Ti.Database.open(menoDatabazy);
                                            db.execute('DELETE FROM annotations WHERE mark_for_del = 1 AND uid=\''+myNickname+'\'');
                ...
                var tbdi = 0;
                var db = Ti.Database.open(menoDatabazy);
                var select = db.execute('SELECT * FROM annotations WHERE mark_for_push=1 AND gmlID!=0 AND uid=\''+myNickname+'\'');
                while (select.isValidRow()) {
                    spl = select.fieldByName('gmlID').split(".");
                    var iid = spl[1];
                                ...
                                if (xhr.status == 200) {
                                    ...
                                    if (obj != 0) {
                                        ...
                                        if (obj == "updatedOK") {
                                            ...
                                            db.execute('UPDATE annotations SET mark_for_push=0 WHERE mark_for_push = 1 AND uid=\''+myNickname+'\'');
                                            divider('UPDATE annotations SET mark_for_push=0 WHERE mark_for_push = 1 AND uid=\''+myNickname+'\'');
                                            db.close();
                                            //loaderViewDataFunc();
                                            //Ti.api.info("toRefreshMainWindow if is true");
                                            //buttonAddToMapFunc();
                ...
                var tbdi = 0;
                var db = Ti.Database.open(menoDatabazy);
                var select = db.execute('SELECT * FROM annotations WHERE gmlID=0 AND uid=\''+myNickname+'\'');
                while (select.isValidRow()) {
                    spl = select.fieldByName('gmlID').split(".");
                    var iid = spl[1];
                    ...
                    var tbdfields = [];
                    for (loopI = 0; loopI < select.fieldCount; loopI++) {
                        tbdfields.push(select.field(loopI)); // UMRELI DO RANA- hehe
                    }
                                ...
                                if (xhr.status == 200) {
                                    ...
                                    if (obj != 0) {
                                        ...
                                        if (obj == "insertOK") {
                                            ...
                                            db.execute('DELETE FROM annotations WHERE gmlID = 0 AND uid=\''+myNickname+'\'');
                                            db.close();
                                            //loaderViewDataFunc();
