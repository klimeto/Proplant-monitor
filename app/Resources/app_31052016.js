...
function eventListenerTiGeolocation(e)
{
    ...
    //alert(longitude + " :: " + latitude + " (acc: "+accuracy+"m / "+dateTime(timestamp)+")");
  }
...
function pushOneImage(){
                    ...
                    var db = Ti.Database.open(menoDatabazy);
                    var select = db.execute('SELECT * from annotations WHERE uid=\''+myNickname+'\' AND imageDeviceId=\''+myDeviceId+'\' AND imagePathLocal!=\'\' AND imagePathLocal!=\'null\' AND (imagePathServer=\'\' OR imagePathServer=\'null\') LIMIT 1');
                    var select_all = db.execute('SELECT * from annotations WHERE uid=\''+myNickname+'\' AND imageDeviceId=\''+myDeviceId+'\' AND imagePathLocal!=\'\' AND imagePathLocal!=\'null\' AND (imagePathServer=\'\' OR imagePathServer=\'null\')');
                    db.close();
	 ...
	 */
		...
		var WinEdit = new Window(title, false, true, function (evt) {
				...
				if (evt.source.title == 'Save') {
                    ...
                    var sqlString = "UPDATE annotations SET plnt_name='%1', image='%2', imagePathLocal='%7', imagePathServer='%8', obs_date='%3', crtn='%4', site_conf='%5', site_descr='%6', mark_for_del='0' WHERE id='" + currentFeatIdIn + "'";
